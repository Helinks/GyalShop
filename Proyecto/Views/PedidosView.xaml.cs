...
namespace Proyecto.Views
{
    public partial class PedidosView : UserControl
    {
        ...
        private void dgPedido_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                dgProductosPedido.ItemsSource =
                    productoPorPedidoController.GetProductoPorPedido(selectedPedido);
            }
        }

        private void dgProductoPedido_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
        ...
        private void AgregarCarrito(object sender, RoutedEventArgs e)
        {
            if (numCantidad.Value == 0)
            {
                MessageBox.Show("Por favor escoja la cantidad del producto");
                return;
            }

            if (selectedCarrito == null)
            {
                MessageBox.Show("No se ha escogido ningún producto");
                return;
            }
            ...
            Carrito itemExistente =
                carritoController
                .GetCarrito(SesionService.UsuarioActual)
                .FirstOrDefault(x => x.IdProducto == selectedCarrito.IdProducto);
            ...
            else
            {
                carritoCliente = new Carrito(
                    0,
                    selectedCarrito.IdProducto,
                    SesionService.UsuarioActual.IdUsuario,
                    selectedCarrito.NombreProducto,
                    selectedCarrito.PrecioUnidad,
                    cantidad
                );
        ...
        {
            var listaCarrito =
                carritoController.GetCarrito(SesionService.UsuarioActual);
        ...
        private void BtnFinalizarCompra_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (result == MessageBoxResult.Yes)
            {
                ...
                productoPorPedidoController.SetProductoPorPedido(
                    pedido,
                    carritoList
                );
